	...
	async ({command, type}: {command?: string; type?: 'lint' | 'test' | 'build' | 'custom'}) => {
		...
		if (!execCmd) {
			if (type === 'lint') {
				...
				} else if (fs.existsSync(path.join(ws, 'pyproject.toml'))) {
					execCmd = 'ruff check .';
				}
			} else if (type === 'test') {
				if (fs.existsSync(path.join(ws, 'package.json'))) {
			...
			} else if (type === 'build' && fs.existsSync(path.join(ws, 'package.json'))) {
				execCmd = 'npm run build';
			}
		}

		if (!execCmd) {
			return 'Error: No command provided and could not auto-detect for type.';
		}
