...
const SYMBOL_PATTERNS: Record<string, RegExp> = {
	typescript: /(?:export\s+)?(?:class|interface|type|enum|function|const|let|var)\s+([a-zA-Z_$][\w$]*)/g,
	javascript: /(?:export\s+)?(?:class|function|const|let|var)\s+([a-zA-Z_$][\w$]*)/g,
	python: /(?:class|def)\s+([a-zA-Z_]\w*)/g,
	go: /(?:type|func|const|var)\s+([a-zA-Z_]\w*)/g,
	rust: /(?:pub\s+)?(?:struct|enum|trait|type|fn|const|static)\s+([a-zA-Z_]\w*)/g,
};
...
const searchSymbolsTool = tool(
	async ({query, path: targetPath = '.'}: {query: string; path?: string}) => {
		try {
			...
			return truncate(results.join('\n') || 'No matching symbols found.', 50_000);
		} catch (err) {
