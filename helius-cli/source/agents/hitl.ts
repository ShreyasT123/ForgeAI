...
export class HITLDecision {
  ...
  toResumePayload(): Record<string, unknown> {
    ...
    if (this.editedCommand) payload.edited_command = this.editedCommand;
    if (this.rejectionReason) payload.reason = this.rejectionReason;
    return payload;
  }
}

export interface HITLPresenter {
...
export async function handleHitlInterrupt(
  ...
  if (!state.tasks || state.tasks.length === 0) return null;

  const task = state.tasks[0];
  if (!task.interrupts || task.interrupts.length === 0) return null;
  ...
  if (provided) {
    if (expected && safeTokenCompare(provided, expected)) {
