...
function streamLogs(jobId: string, data: Buffer, prefix: string, maxLines: number): void {
  ...
  const buf = JOB_LOGS.get(jobId) as string[];
...
const runCommandTool = tool(
  async ({
    ...
    if (background) {
      ...
      proc.stdout?.on("data", (data) => streamLogs(jobId, data, "OUT", maxLines));
      proc.stderr?.on("data", (data) => streamLogs(jobId, data, "ERR", maxLines));
      proc.on("close", () => {
