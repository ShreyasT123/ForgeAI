...
function ensureFile(target: string, label: string): [boolean, string] {
  if (!fs.existsSync(target)) return [false, `Error: '${label}' does not exist.`];
  const stat = fs.statSync(target);
  if (!stat.isFile()) return [false, `Error: '${label}' is not a file.`];
  return [true, ""];
}
  ...
  for (const op of ops) {
    if (op.type === "equal") {
      if (hunk) {
        ...
        buffer.push(op);
        if (buffer.length > context) buffer.shift();
      }
      oldLine += 1;
      newLine += 1;
    } else {
...
const lsTool = tool(
  async ({ path: targetPath = "." }: { path?: string }) => {
    try {
      ...
      if (!fs.existsSync(target)) return `Error: '${targetPath}' does not exist.`;
...
const overwriteFileTool = tool(
  async ({ path: filePath, content }: { path: string; content: string }) => {
    try {
      ...
      if (newContent === current) return `No changes needed in '${filePath}'.`;
...
const applyPatchTool = tool(
  async ({ path: filePath, patch }: { path: string; patch: string }) => {
    try {
      ...
      if (updated === content) return `Patch produced no changes in '${filePath}'.`;
...
const grepSearchTool = tool(
  async ({
    ...
    try {
      ...
      if (!fs.existsSync(target)) return `Error: '${searchPath}' does not exist.`;
      let regex: RegExp;
      try {
      ...
      const walk = (dir: string): boolean => {
        ...
        for (const entry of entries) {
          if (entry.isDirectory()) {
            if (cfg.ignore_dirs.has(entry.name)) continue;
            if (walk(path.join(dir, entry.name))) return true;
          } else if (entry.isFile()) {
            if (grepFile(path.join(dir, entry.name))) return true;
          }
        }
        return false;
      };
